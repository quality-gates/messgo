...
- **paths** — comma-separated files or directories. Directories are walked;
  `vendor/`, `node_modules/`, `.git/`, and `testdata/` are skipped. A path ending in `...`
  (Go's recursive wildcard, e.g. `./...`) is walked from that directory. Explicit
  paths under `testdata/` stay analyzable.
